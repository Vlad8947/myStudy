...
public class PBESample {
    ...
    public byte[] encrypt(String algorithm, int keySize, final byte[] message) throws Exception {
        ...
        // to the algorithm block size (if it is a block algorithm).
    ...
    public byte[] decrypt(String algorithm, int keySize, final byte[] encryptedMessage) throws Exception {
        ...
        final int saltSize = (Math.min(saltSizeBytes, decryptedMessage.length));
        final int ivStart = (Math.min(saltSizeBytes, decryptedMessage.length));
        final int ivSize = cipher.getBlockSize();
        final int encMesKernelStart = (Math.min(saltSizeBytes + ivSize, decryptedMessage.length));
        final int encMesKernelSize =
                (saltSizeBytes + ivSize < decryptedMessage.length ?
                        (decryptedMessage.length - saltSizeBytes - ivSize) : 0);
